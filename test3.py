...
def compute_velocity_from_streamfunction(psi, dx):
    u = np.gradient(psi, axis=1) / dx     # ∂ψ/∂y
    v = -np.gradient(psi, axis=0) / dx    # -∂ψ/∂x
    return u, v
...
def simulate_heat_trajectory(u0, dt, t_end, g, A, add_noise=False):
...
def ddim_reverse_step(x, model, t, t_prev, A):
    with torch.no_grad():
        ...
        heat_term = torch.from_numpy(heat).unsqueeze(0).unsqueeze(0).to(x.device)
        return x + (t - t_prev) * (score + heat_term)
...
def visualize_spectrum_triple(u0, recon, uT):
    ...
    plt.loglog(k0, E0,  label='Ground Truth u₀')
    plt.loglog(kr, Er,  label='Reconstructed û₀')
    plt.loglog(kf, Ef,  label='Final u_T')
    plt.xlabel("Wave number $k$")
    plt.ylabel("Energy Spectrum $E(k)$")
    plt.title("2D Isotropic Energy Spectra")
    plt.legend(); plt.grid(True, which="both", ls="--")
    plt.tight_layout(); plt.savefig("energy_spectra_comparison.png"); plt.show()
...
def main():
    ...
    forward   = simulate_heat_trajectory(u0, dt, T, g, A, add_noise=False)
    x_final   = forward[-1]

    model     = UNet().to(device)
    if not os.path.exists("trained_model.pt"):
        print("[Error] trained_model.pt not found. Please train first.")
        return
    ...
    v_recon    = np.zeros_like(u_recon)   # 仅重建横向分量，纵向置零
